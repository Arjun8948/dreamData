...
const Banner = () => {
  ...
  const getAdBanner = () => {
    setLoading(true);
    axios.get(`https://dearmschool.com/api/getbanner/all`)
      .then((res) => {
  ...
  const deleteHandler = (id) => {
    axios.delete(`https://dearmschool.com/api/bannerimg?id=${id}`)
      .then((res) => {
        massage.success({
          message: `Banner Ads  Deletion Success`,
      ...
      .catch((err) => {
        massage.error({
          message: ` Banner  Ads Deletion field`,
          description: `${
  ...
  const restult = adBanner.map((item, ind) => {
    const data = {
     
      no: ind+1,
      massage: item.massage,
      bannerImg:<img src={item.imgUrl}  height={"100px"} width={'auto'}/> ,
      detete: (
        <Button
          type="primary"
  ...
  return (
    <div className={bannerStyle.mainSection}>
      ...
      <div className={bannerStyle.featureSection}>
        ...
        >
          {adBanner.map((item) => (
            ...
            >
              <img src={item.imgUrl} alt=""  style={{height:"100%" ,width:"100%",borderRadius:"3px"}}/>
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
      <div className={bannerStyle.sectionBtn}>
        ...
        <AddBanner getAdBanner={getAdBanner} />
      </div>
...
const AddBanner = ({ getAdBanner }) => {
  ...
  const handleOk = () => {
    ...
    formData.append("banner", profileImg);
  setLoading(true);
    axios
      .post(`https://dearmschool.com/api/banner`, formData, {
      ...
      .then((res) => {
        massage.success({
          message: ` Banner Ads Upload Success`,
          description: `${res.data.massage}`,
          placement: "top",
        });
        setLoading(false);
    ...
    <>
      ...
      >
         <div style={{display:"flex",flexDirection:"column"}}>
                ...
                <input style={InputStyle} type={resetFile} name="banner" onChange={(e)=>setProfileImg(e.target.files[0])} />
               {
